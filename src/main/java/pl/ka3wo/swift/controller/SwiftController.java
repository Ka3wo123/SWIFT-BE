...
public class SwiftController {
    ...
    public ResponseEntity<SwiftDataDto> getBySwiftCode(@PathVariable("swift-code") String swiftCode) {
        SwiftDataDto bySwiftCode = swiftService.getBySwiftCode(swiftCode);
        return ResponseEntity.ok(bySwiftCode);
    }

    @GetMapping("/country/{countryISO2code}")
    public ResponseEntity<SwiftDataCountryDto> getByCountryISO2code(@PathVariable("countryISO2code") String countryISO2code) {
    ...
    public ResponseEntity<ApiResponse> deleteBySwiftCode(@PathVariable("swift-code") String swiftCode) {
        return ResponseEntity.ok(new ApiResponse("Data deletion acknowledged. Swift code " + swiftCode  ));
    }
}
